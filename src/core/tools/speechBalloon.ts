...
export class SpeechBalloonShape extends BaseShapeTool{
    ...
    private transformControlPoints(placement:SpeechBalloonPlacement) {
    ...
    private computDrawPoints(thickness:number, placement:SpeechBalloonPlacement):{pos: [number,number], rect: IRectType, points:Vec2d[]} {
        const r = getRectFromPoints(this.tmpPoints);
        ...
        const pos:[number,number] = [Math.floor(rect.x + rect.w / 2), Math.floor(rect.y + rect.h / 2)];
        return {
            rect,
            pos,
            points
    ...
    static updateNodeOpt(param:{
        ...
        const _Opt = nodeOpt?.opt as SpeechBalloonOptions;
        let n = node;
