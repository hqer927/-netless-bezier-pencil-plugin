...
export class StarShape extends BaseShapeTool{
    readonly canRotate: boolean = false;
    readonly scaleType: EScaleType = EScaleType.all;
    readonly toolsType: EToolsKey = EToolsKey.Star;
    ...
    constructor(props:BaseShapeToolProps) {
        super(props);
        this.workOptions = props.toolsOpt as StarOptions;
    ...
    consume(props: { data: IWorkerMessage; isFullWork?: boolean | undefined; isClearAll?: boolean | undefined; isSubWorker?: boolean | undefined; }): IMainMessage {
        ...
        const rect: IRectType | undefined = this.draw({workId, layer, isDrawing:true});
        const r = computRect(rect, this.oldRect);
        this.oldRect = rect;
        return {
            rect: r,
            type: EPostMessageType.DrawWork,
    ...
    private draw(props:{
        ...
        const {strokeColor, fillColor, thickness, zIndex, vertices, innerVerticeStep, innerRatio, scale, rotate, translate} = this.workOptions;
        const worldPosition = layer.worldPosition;
        const worldScaling = layer.worldScaling;
        const {rect, pos, points} = this.computDrawPoints(thickness, vertices, innerVerticeStep, innerRatio);
        const attr:any = {
            pos,
            close:true,
            name: workId,
    ...
    private computDrawPoints(thickness:number, vertices:number, innerVerticeStep:number, innerRatio:number):{pos: [number,number], rect: IRectType, points:number[]} {
        ...
        const outerRadius = Math.floor(Math.min(r.w,r.h) / 2);
        const innerRadius = innerRatio * outerRadius;
        const points:number[] = [];
        const average = 2 * Math.PI / vertices;
        for(let i = 0; i < vertices; i++) {
            const angle = i * average - 0.5 * Math.PI;
            let x,y;
            if (i % innerVerticeStep === 1) {
                x = innerRadius * scale[0] * Math.cos(angle);
                y = innerRadius * scale[1] * Math.sin(angle);
            } else {
                x = outerRadius * scale[0] * Math.cos(angle);
                y = outerRadius * scale[1] * Math.sin(angle);
                points.push(x,y);
            }
            points.push(x,y);
        }
    ...
    private updateTempPoints(op:number[]):boolean{
        ...
        const {thickness} = this.workOptions as StarOptions;
    ...
    static updateNodeOpt(param:{
        ...
        const _Opt = nodeOpt?.opt as StarOptions;
        let n = node;
